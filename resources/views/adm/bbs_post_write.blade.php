...
<div class="row">
        ...
        <form method="POST" id="post_create" action="{{ route('post.create') }}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="user_id" value="{{ Auth::user()->id }}"> {{-- 작성자 id --}}
            <input type="hidden" id="prev_url" name="prev_url" value="{{ $prev_url }}"> {{-- 이전페이지 url --}}
            <input type="hidden" name="board_name" value="{{ !isset($select_board->title) ? '' : $select_board->title }}"> {{-- board key --}}
            <div class="card">
                    ...
                    <div class="row">
                        <table class="table " id="info_table">
                            <tbody>
                                ...
                                <tr>
                                    <td>제목</td>
                                    <td><input type="text" class="form-control form-control-sm col-3" name="title" value="" autocomplete='off'></td>
                                </tr>
                                <tr>
                                    <td>조회수</td>
                                    <td><input type="text" class="form-control form-control-sm col-3" name="view_count" value="" autocomplete='off'></td>
                                </tr>
                                <tr>
                                    <td>작성일</td>
                                    <td>
                                        <div class="form-group col-3" style="margin:0; padding:0">
                                            <div class="input-group input-group-sm date" id="reservationdatetime" data-target-input="nearest">
                                                <input type="text" class="form-control datetimepicker-input" data-target="#reservationdatetime" name="create_date" value="" autocomplete='off'/>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="input-group input-group-sm col-3" style="margin:0; padding:0">
                                            <input type="text" class="form-control" name="nickname" value="" autocomplete='off'>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                @if(isset($select_board) && $select_board->use_notice == 'Y')
                                    ...
                                    <tr id="thumb_tr" >
                                        <td>공지사항 등록</td>
                                        <td>
                                            <div class="part"><input type="checkbox" name="notice" value="Y" style="width:20px; height:20px"></div>
                                        </td>
                                    </tr>
                                @endif
                                <tr id="thumb_tr" >
                                        <td>비밀글로 등록</td>
                                        <td>
                                            <div class="part"><input type="checkbox" name="is_view" value="N" style="width:20px; height:20px"></div>
                                        </td>
                                    </tr>
                                <tr>
                                    <td>내용</td>
                                    <td>
                                        <textarea name='content' id='content' STYLE='width:100%;height:600px;'></textarea>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ $prev_url }}" class="btn btn-default float-right">목록으로</a>
                </div>
            </div><!-- /.card END-->
            </form>
        </div>
...
<!-- SmartEditor 관련 -->
    ...
    @if(isset($select_board) && $select_board->title != 'review')
...
<script>
    // 게시글 작성 전에 게시판 먼저 선택 유도
    $(":input").click(function(){       
        if($(this).attr('id') != 'board_id' && $("#board_id").val() == "") {
            alert('게시판을 먼저 선택해 주세요.');
            $("#board_id").css('border','red solid 1px');
        }
    });
    ...
    function boardForm(){
        ...
        location.href="/admin/bbs_manage/write?board_id="+$board_id+"&prev_url="+$("#prev_url").val();
    }
</script>
