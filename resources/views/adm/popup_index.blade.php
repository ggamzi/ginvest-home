...
<section class="content">
    <div class="card col">
        ...
        <div class="card-header">
            <div class="row">
                <div class="col-auto">
                    <button class="btn btn-outline-info btn-sm" data-toggle="modal" data-target="#pop_create">팝업 생성</button>
                </div>
                <div class="col-auto">
                    <button class="btn btn-outline-primary btn-sm" id="order_change">
                        노출순서 변경
                        <span>드래그앤 드롭으로 순서를 변경한 후 버튼을 눌러주세요. <br>노출 순서는 위쪽에 있을수록 우선순위가 높습니다.</span>
                    </button>
                </div>
            </div>
        </div>
        <div class="card-body">
            <div class="row">
                <table class="table table-sm table-bordered text-center table-hover" id="popup_list">
                    <thead class="bg-gray">
                        <th>순서</th>
                        <th>설명</th>
                        <th>링크</th>
...
<script>
    ...
    $("#popup_list").tableDnD({
        ...
        onDrop: function(table, row) {
            popup_order = jQuery.tableDnD.serialize();
            console.log(popup_order);
        }
    })

    $("#order_change").click(function(){
        if (popup_order == '') {
            alert('순서가 변경되지 않았습니다.');
            return false;
        }
        if (confirm('순서를 바꾸시겠습니까?') == false) return false;
        location.href="{{ route('popup.order.update') }}?"+popup_order;
    });
